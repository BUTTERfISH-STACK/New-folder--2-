...
function saveCVData() {
    ...
    localStorage.setItem('vellonx_cv_data', JSON.stringify(state.cvData));
}
...
function downloadCertificate() {
    ...
    // Verification text
    doc.setFontSize(8);
    doc.text('VERIFICATION: Visit vellonx.ai/verify and enter Certificate ID above', pageWidth / 2, y, { align: 'center' });
...
function logout() {
    state.user = null;
    localStorage.removeItem('vellonx_user');
    updateUI();
    showSection('home');
...
function saveUserData() {
    ...
    localStorage.setItem('vellonx_user_data', JSON.stringify(userData));
}

function loadUserData() {
    const saved = localStorage.getItem('vellonx_user_data');
    if (saved) {
...
function saveAISettings() {
    ...
    // Save to localStorage
    localStorage.setItem('vellonx_ai_token', token);
...
function loadAISettings() {
    const savedToken = localStorage.getItem('vellonx_ai_token');
    if (savedToken) {
